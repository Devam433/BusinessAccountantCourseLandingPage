...
<section id="testimonials" className="py-24 px-6 md:px-10 overflow-hidden">
  <div className="max-w-7xl mx-auto">
    <div className="text-center mb-16 space-y-4">
      <div className="inline-flex items-center px-4 py-2 bg-[#F4F2E3] text-[#b09458] rounded-full text-sm font-medium border border-[#F5D951]">
        <Star className="mr-2 h-4 w-4" />
        Student Success Stories
      </div>
      <h2 className="text-3xl md:text-4xl font-display font-bold text-[#000000]">
        Hear From Our <span className="text-[#f5d951]">Graduates</span>
      </h2>
      <p className="text-lg text-[#000000]/70 max-w-2xl mx-auto">
        Our students come from diverse backgrounds but share a common goal:
        to become professional Business Accountant
      </p>
    </div>
    ...
    <div className="relative">
          ...
          >
            {group.map((testimonial, i) => (
              <div key={i} className="w-full md:w-1/2 lg:w-1/3 px-4">
                <div className="h-full bg-white p-8 rounded-xl shadow-md border border-[#F4F2E3] flex flex-col">
                  <div className="mb-6 text-[#F5D951]">
                    <Quote className="h-10 w-10 opacity-20" />
                  </div>
                  <p className="flex-grow text-[#000000]/80 italic mb-6">
                    "{testimonial.content}"
                  </p>
                  <div className="flex items-center">
                    ...
                    />
                    <div>
                      <div className="font-medium text-[#000000]">{testimonial.name}</div>
                      <div className="text-sm text-[#000000]/60">{testimonial.role}</div>
                      <div className="flex mt-1">
                        {[...Array(testimonial.rating)].map((_, i) => (
                          <Star key={i} className="h-4 w-4 fill-[#F5D951] text-[#F5D951]" />
                        ))}
                      </div>
                    </div>
                  </div>
      ...
      <div className="flex justify-center mt-8 gap-4">
        <button
          onClick={prevSlide}
          className="bg-white border border-[#F4F2E3] p-2 rounded-full shadow hover:bg-[#F4F2E3] transition"
          aria-label="Previous"
        >
          ‹
        </button>
    ...
    <div className="mt-16 grid md:grid-cols-3 gap-8">
      <div className="bg-white p-6 rounded-xl shadow-md border border-[#F4F2E3] text-center">
        <div className="text-4xl font-bold text-[#F5D951] mb-2">100%</div>
        <p className="text-[#000000]/70">Job Placement Rate</p>
      </div>
      <div className="bg-white p-6 rounded-xl shadow-md border border-[#F4F2E3] text-center">
        <div className="text-4xl font-bold text-[#F5D951] mb-2">200+</div>
        <p className="text-[#000000]/70">Graduates</p>
      </div>
      <div className="bg-white p-6 rounded-xl shadow-md border border-[#F4F2E3] text-center">
        <div className="text-4xl font-bold text-[#F5D951] mb-2">20k+</div>
        <p className="text-[#000000]/70">Average Starting Salary</p>
      </div>
    </div>
  </div>
</section>
